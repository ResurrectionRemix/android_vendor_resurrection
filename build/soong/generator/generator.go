...
func init() {
	android.RegisterModuleType("rr_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = rrExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = rrExpandVariables(ctx, dep_file)
	...
	cmd := rrExpandVariables(ctx, String(g.properties.Cmd))
