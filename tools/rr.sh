...
echo -e "";
echo "{ \"response\": [ { \"datetime\": $(grep ro\.build\.date\.utc $OUT/system/build.prop | cut -d= -f2), \"filename\": \"$(basename $(ls $OUT/RR-*.zip))\", \"id\": \"$((sha256sum $OUT/RR-*.zip) | cut -d ' ' -f1)\", \"romtype\": \"$RR_BUILDTYPE\", \"size\": $(stat -c%s $OUT/RR-*.zip), \"url\": \"https://downloads.sourceforge.net/project/resurrectionremix-pie/$RR_BUILD/$(basename $(ls $OUT/RR-P*.zip))\", \"version\": \"$(grep -m1 ro\.rr\.build\.version $OUT/system/build.prop | cut -d= -f2)\"  }]}" | tee $OUT/$RR_BUILD.json
